...
export class PirateBeachManager {
  ...
  public static readonly SPAWN_X = 2005;
  public static readonly SPAWN_Z = 1995;
  public static readonly WAYGATE_POS = new THREE.Vector3(2012, 0, 1990);
   ...
   * Smooth crescent bay on the west (X < 1990) and dramatic sandstone cliffs on the east (X > 2025)
   */
  ...
  public getSpawnPosition(): THREE.Vector3 {
    const x = PirateBeachManager.SPAWN_X;
    const z = PirateBeachManager.SPAWN_Z;
    const y = this.sampleHeight(x, z) + 1.2;
  ...
  public buildBeach(): void {
    ...
    // 280m x 280m bespoke landscape with world-space vertex positioning & Rapier Trimesh
    // ==========================================
    ...
    for (let zi = 0; zi <= segments; zi++) {
      for (let xi = 0; xi <= segments; xi++) {
        const wx = originX - halfSize + xi * step;
        const wz = originZ - halfSize + zi * step;
        const h = this.sampleHeight(wx, wz);

        // Put exact world-space coordinates directly into vertex buffer
        positions[vIdx] = wx;
        positions[vIdx + 1] = h;
        positions[vIdx + 2] = wz;
    ...
    beachGeo.setIndex(new THREE.BufferAttribute(indices, 1));
    ...
    // Mesh is in world coordinates directly
    const terrainMesh = new THREE.Mesh(beachGeo, beachMat);
    terrainMesh.position.set(0, 0, 0);
    terrainMesh.receiveShadow = true;
    this.beachGroup.add(terrainMesh);

    // Dedicated Rapier Trimesh Collider: 100% exact 1:1 match to visual surface
    ...
    // Extends from the beach waterline (X = 1992, Z = 1990) out into ocean bay (X = 1957, Z = 1990)
    // ==========================================
    const pierZ = 1990;
    const dockY = 0.85;

    // Modular wooden pier sections
    for (let x = 1990; x >= 1962; x -= 7) {
    ...
    this.addPiece(factory, "Environment_Dock", 1957, dockY, pierZ - 5, 0, 1.8);
    this.addPiece(factory, "Environment_Dock", 1957, dockY, pierZ + 5, 0, 1.8);
    this.addPiece(factory, "Environment_Dock_Broken", 1957, dockY, pierZ - 11, 0, 1.6);

    // Walkable deck collider for the main pier (X in [1957, 1992])
    const pierCollider = this.physicsWorld.createCollider(
      RAPIER.ColliderDesc.cuboid(18.0, 0.25, 2.2).setRestitution(0.0).setFriction(1.2)
    );
    pierCollider.setTranslation({ x: 1974.5, y: dockY - 0.15, z: pierZ });
    this.colliders.push(pierCollider);

    // Pier T-head platform collider
    const tHeadCollider = this.physicsWorld.createCollider(
      RAPIER.ColliderDesc.cuboid(2.4, 0.25, 9.0).setRestitution(0.0).setFriction(1.2)
    );
    tHeadCollider.setTranslation({ x: 1957.0, y: dockY - 0.15, z: pierZ });
    this.colliders.push(tHeadCollider);

    // Pier props
    this.addPiece(factory, "Prop_Bucket_Fishes", 1985, dockY + 0.1, pierZ + 1.2, 0.4, 1.3);
    this.addPiece(factory, "Prop_Barrel", 1976, dockY + 0.1, pierZ - 1.1, 0, 1.2);
    this.addPiece(factory, "Prop_Barrel", 1975.2, dockY + 0.1, pierZ - 1.0, 0.8, 1.1);
    this.addPiece(factory, "Prop_Bottle_1", 1975.8, dockY + 0.9, pierZ - 1.1, 0.2, 1.2);
    this.addPiece(factory, "Prop_Anchor", 1958.0, dockY + 0.1, pierZ + 6.0, Math.PI / 4, 1.5);
    this.addPiece(factory, "Prop_Cannon", 1958.0, dockY + 0.1, pierZ - 6.0, -Math.PI / 2, 1.3);

    // ==========================================
    // 3. THE BLACK PEARL PIRATE GALLEON (Ship_Large)
    // Anchored beside the pier platform
    // ==========================================
    const shipX = 1942;
    const shipZ = 1990;
    this.shipBaseY = -1.1;
    ...
    // Gangplank connecting Pier T-head (X = 1957) to Ship Boarding Deck (X = 1950)
    ...
    const gangplankCol = this.physicsWorld.createCollider(
      RAPIER.ColliderDesc.cuboid(3.8, 0.2, 1.6).setRestitution(0.0).setFriction(1.2)
    );
    gangplankCol.setTranslation({ x: 1953.5, y: dockY + 0.15, z: pierZ });
    this.colliders.push(gangplankCol);

    // Ship Walkable Colliders
    ...
    this.addPiece(factory, "Ship_Small", 1925, -0.6, 2030, -0.7, 1.6);

    // ==========================================
    // 4. THE KRAKEN ENCOUNTER (Characters_Tentacle)
    // Giant tentacles breaching the waves
    // ==========================================
    const tentacle1 = this.addPiece(factory, "Characters_Tentacle", 1930, -1.2, 1999, 0.4, 2.4);
    if (tentacle1) this.tentacles.push(tentacle1);

    const tentacle2 = this.addPiece(factory, "Characters_Tentacle", 1936, -1.5, 1962, -1.8, 2.0);
    if (tentacle2) this.tentacles.push(tentacle2);

    const tentacle3 = this.addPiece(factory, "Characters_Tentacle", 1916, -1.8, 2018, 2.2, 2.6);
    ...
    // Placed across the golden sand dunes and bluffs
    // ==========================================
    const palmLocations = [
      { x: 2008, z: 2012, type: "Environment_PalmTree_1", scale: 1.5, rot: 0.5 },
      { x: 2014, z: 2020, type: "Environment_PalmTree_2", scale: 1.7, rot: 1.2 },
      { x: 2004, z: 1978, type: "Environment_PalmTree_3", scale: 1.6, rot: 2.1 },
      { x: 2010, z: 1965, type: "Environment_PalmTree_1", scale: 1.8, rot: 3.0 },
      { x: 2020, z: 1955, type: "Environment_PalmTree_2", scale: 1.5, rot: 0.8 },
      { x: 2024, z: 1970, type: "Environment_PalmTree_3", scale: 1.6, rot: 1.7 },
      { x: 2018, z: 1982, type: "Environment_PalmTree_1", scale: 1.4, rot: 2.4 },
      { x: 2012, z: 2030, type: "Environment_PalmTree_2", scale: 1.7, rot: 0.2 },
      { x: 2018, z: 2042, type: "Environment_PalmTree_1", scale: 1.5, rot: 1.9 },
      { x: 2026, z: 2050, type: "Environment_PalmTree_3", scale: 1.6, rot: 2.8 },
      { x: 2022, z: 2064, type: "Environment_PalmTree_1", scale: 1.4, rot: 0.9 },
      { x: 2002, z: 1945, type: "Environment_PalmTree_2", scale: 1.5, rot: 1.4 },
      { x: 2000, z: 2055, type: "Environment_PalmTree_3", scale: 1.7, rot: 2.5 },
    ];
    ...
    // On the southern sandbar jutting into the surf at X = 1995, Z = 2038
    // ==========================================
    const skullH = this.sampleHeight(1995, 2038);
    this.addPiece(factory, "Environment_Skulls", 1995, Math.max(0.2, skullH + 0.2), 2038, -0.6, 2.2);
    const skullCol = this.physicsWorld.createCollider(
      RAPIER.ColliderDesc.cuboid(3.5, 3.5, 3.5).setRestitution(0.0).setFriction(1.2)
    );
    skullCol.setTranslation({ x: 1995.0, y: Math.max(2.5, skullH + 2.5), z: 2038.0 });
    this.colliders.push(skullCol);

    const boneH = this.sampleHeight(2000, 2028);
    this.addPiece(factory, "Environment_LargeBones", 2000, Math.max(0.1, boneH), 2028, 0.4, 2.0);

    // ==========================================
    // 8. BEACH PIRATE CAMP & BURIED TREASURE
    // Between X = 2006 and 2016 on the warm sand dunes
    // ==========================================
    // Beached rowboat tilted in the sand
    const boatH = this.sampleHeight(2006, 2008);
    const boat = this.addPiece(factory, "Ship_Small", 2006, boatH + 0.3, 2008, 0.4, 1.2);
    if (boat) boat.rotation.z = 0.18;

    // Open treasure chest overflowing with gold
    const chestH = this.sampleHeight(2012, 2005);
    this.addPiece(factory, "Prop_Chest_Gold", 2012, chestH + 0.05, 2005, 0.3, 1.4);
    this.addPiece(factory, "Prop_Coins", 2012.8, chestH + 0.02, 2005.2, 0.8, 1.6);
    this.addPiece(factory, "Prop_Coins", 2011.4, chestH + 0.02, 2004.7, 1.5, 1.4);
    this.addPiece(factory, "Prop_GoldBag", 2013.0, chestH + 0.05, 2004.4, 0.4, 1.3);

    // Closed secondary chest
    const chest2H = this.sampleHeight(2014, 1998);
    this.addPiece(factory, "Prop_Chest_Closed", 2014, chest2H + 0.05, 1998, -0.4, 1.3);

    // Shore defense cannons
    const cannon1H = this.sampleHeight(2004, 1980);
    this.addPiece(factory, "Prop_Cannon", 2004, cannon1H + 0.05, 1980, -Math.PI / 2 - 0.2, 1.35);
    this.addPiece(factory, "Prop_CannonBall", 2004.8, cannon1H + 0.05, 1980.6, 0, 1.2);

    const cannon2H = this.sampleHeight(2005, 2018);
    this.addPiece(factory, "Prop_Cannon", 2005, cannon2H + 0.05, 2018, -Math.PI / 2 + 0.3, 1.35);

    // Rum barrels and bottles
    const barrelH = this.sampleHeight(2013, 2002);
    this.addPiece(factory, "Prop_Barrel", 2013, barrelH + 0.05, 2002, 0, 1.2);
    this.addPiece(factory, "Prop_Barrel", 2013.8, barrelH + 0.05, 2002.2, 0.4, 1.1);
    this.addPiece(factory, "Prop_Bottle_1", 2012.6, barrelH + 0.05, 2002.5, 0.7, 1.2);

    // Giant beach anchor stuck in sand
    const anchorH = this.sampleHeight(2002, 1970);
    const beachAnchor = this.addPiece(factory, "Prop_Anchor", 2002, anchorH + 0.3, 1970, 0.8, 1.8);
    if (beachAnchor) beachAnchor.rotation.x = -0.3;

    // Skeleton guard & Sharky pirate
    const skelH = this.sampleHeight(2011.5, 2006.5);
    this.addPiece(factory, "Characters_Skeleton", 2011.5, skelH + 0.05, 2006.5, -0.8, 1.2);

    const sharkyH = this.sampleHeight(2001, 1986);
    this.addPiece(factory, "Characters_Sharky", 2001, sharkyH + 0.05, 1986, -Math.PI / 2 + 0.2, 1.3);

    // Smooth shoreline rocks
    const rocks = [
      { x: 1998, z: 2010, type: "Environment_Rock_1", scale: 1.8 },
      { x: 2000, z: 1982, type: "Environment_Rock_2", scale: 2.2 },
      { x: 1996, z: 1965, type: "Environment_Rock_3", scale: 2.5 },
      { x: 1997, z: 2026, type: "Environment_Rock_4", scale: 1.9 },
      { x: 2001, z: 2045, type: "Environment_Rock_5", scale: 2.1 },
    ];
    ...
    // Stands near the beach entrance to the pier at (2012, 1990)
    // ==========================================
    ...
    this.isBuilt = true;
    console.log("🏴‍☠️ PirateBeachManager: Dedicated Pirate Beach & Ocean Cove built successfully with Trimesh Collider.");
  }
